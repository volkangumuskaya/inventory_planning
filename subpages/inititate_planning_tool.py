...
from data_class_script import Customer, Order, Product, Resource, generate_customers, generate_orders, \
    generate_products, generate_resources
        ...
        if st.button("BUILD MODEL", type="primary"):
            try:
                ...
                products = generate_products(
                    n_products=n_products, resources=resources, min_resource_needed=min_resource_needed,
                    max_resource_needed=max_resource_needed)
                ...
                orders = generate_orders(
                    n_orders=n_orders, products=products, customers=customers, time_periods=time_ids,
                ...
                st.session_state.order_df = pd.DataFrame(
                    [(obj.order_id, obj.customer_name, obj.deadline, obj.product, obj.total_resource_usage) for obj in
                     orders])
                st.session_state.order_df.columns = ['OrderId', 'CustomerName', 'Deadline', 'ProductType-amount',
                                                     'ResourceType-amount']
                ...
                x = LpVariable.dicts(
                    name="resource_production", indices=(resource_ids, time_ids), lowBound=0, upBound=None,
                    cat=LpContinuous)
                # Y: 1 if order O is fulfilled in time T. We allow only complete fulfillment of orders/delivery
                y = LpVariable.dicts(
                    name="order_fulfillment", indices=(order_ids, time_ids), lowBound=0, upBound=None, cat=LpBinary)
                # 1 if order O is delayed in time T
                order_delay = LpVariable.dicts(
                    name="order_delay", indices=(order_ids, time_ids), lowBound=0, upBound=None, cat=LpBinary)
                # starting inventory of resource R in time T
                inv = LpVariable.dicts(
                    name="starting_inventory", indices=(resource_ids, time_ids + [len(time_ids)]),
                ...
                obj_func_delay_cost = [(order_delay[o][t]) * delay_costs[o] for t in time_ids for o in order_ids if
                                       t >= orders[o].deadline]
                ...
                for t in time_ids:
                    for r in resource_ids:
                        prob += (
                            lpSum([orders[o].total_resource_usage[r] * y[o][t] for o in order_ids]) - x[r][t] - inv[r][
                                t] == -inv[r][t + 1],
                ...
                with open('problem.pickle', 'wb') as handle:
                    pickle.dump(
                        [prob, resource_names, product_names, order_names], handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    if st.session_state.show_solve_section:
        with col3:
            if st.button("SOLVE MODEL", type="primary"):
                ...
                df['VarType'] = df['var_name'].apply(
                    lambda x: next(
                        (var_type for prefix, var_type in prefix_to_var_type.items() if x.startswith(prefix)), None))
                st.session_state.df = df.copy()
                st.session_state.summary_df = pd.DataFrame(
                    {'name': ['Total_production_cost', 'Total_delays', 'n_vars', 'n_constraints'],
                     'value': [
